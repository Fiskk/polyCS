...
function create_image_link($image,$text) {
	...
	if( isset($image) AND file_exists("pix/$image") ) {
		...
		return "<a href='pix/$image' rel='helpdoc' class='thickbox' title='$text'><img border=0 src='pix/$image' height=$height></a>\n";
	}
}

$steps[0] = null;
$steps[]['text'] = "Be sure you are running the most recent version of Acrobat Reader.  If not, <a href='http://get.adobe.com/reader/'>download it</a>.";
...
$steps[] = array("image" => "trust2.jpg", "text" => "Open Acrobat Reader and click Document / Manage Trusted Identities.");
...
$steps[] = array("image" => "trust6.jpg", "text" => "Select on Computer Science Dept. CA in the top box, select Computer Science Dept. in the bottom box, then click Details.");
...
for($i=1;$i<=count($steps)-1;$i++) {
	if( isset($steps[$i]['image']) ) echo "<BR>" . create_image_link($steps[$i]['image'],$steps[$i]['text']);
